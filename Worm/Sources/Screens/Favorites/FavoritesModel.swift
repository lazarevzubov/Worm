...
actor FavoritesServiceBasedModel: FavoritesModel {
    ...
    private func updateFavorites(with favoriteBookIDs: Set<String>) async {
        var updatedFavorites = favorites.filter { favoriteBookIDs.contains($0.id) }
        for bookID in favoriteBookIDs where !updatedFavorites.contains(where: { $0.id == bookID }) {
            if let book = await catalogService.getBook(by: bookID) {
                updatedFavorites.append(book)
            }
        }
